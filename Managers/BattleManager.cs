...
public class BattleManager : MonoBehaviour
{
    ...
    public GameObject CreateHeroPrefab(BaseHero hero, Vector3 position)
    {
        // 1. Check for null prefab:
        if (heroPrefab == null)
        {
            Debug.LogError("Hero prefab is not assigned! Please assign a prefab in the Inspector.");
            return null;
        }

        // 2. Instantiate the prefab:
        GameObject heroGameObject = Instantiate(heroPrefab, position, Quaternion.identity);

        // 3. Set name and components:
        heroGameObject.name = hero.heroSO.heroName;

        BaseHero newHero = heroGameObject.GetComponent<BaseHero>();
        if (newHero == null)
        {
            newHero = heroGameObject.AddComponent<BaseHero>();
        }
        newHero.InitializeHero(hero.heroSO);

        HeroStateMachine hsm = heroGameObject.GetComponent<HeroStateMachine>();
        if (hsm == null)
        {
            hsm = heroGameObject.AddComponent<HeroStateMachine>();
        }
        hsm.Initialize(newHero, uiManager);

        // 4. Handle Selector existence gracefully:
        ...
        {
            Debug.LogWarning("Selector not found in hero prefab's children. Functionality might be affected.");
        }
